...
save_if_interesting(afl_state_t *afl, void *mem, u32 len, u8 fault) {
  ...
  u8 tmp_mapsize[MAP_SIZE];
  memset(tmp_mapsize, 0, MAP_SIZE);
  ...
  cksum = hash64(tmp_mapsize, MAP_SIZE, HASH_CONST);

  struct unique_path *p = afl->unique_path;
  u8 is_new = 1;
  ...
  if (is_new) {
    afl->total_unique_path++;
    struct unique_path *new = ck_alloc(sizeof(struct unique_path));
    new->next = NULL;
    new->hash = cksum;
    p->next = new;
  }
